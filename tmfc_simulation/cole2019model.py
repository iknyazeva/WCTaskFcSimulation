...
class NeuralMassModel:
    ...
    def _init_synpatic_cole(self):
        ...
        Cole MW, Ito T, Schultz D, Mill R, Chen R, Cocuzza C (2019). "Task activations produce spurious
         but systematic inflation of task functional connectivity estimates". NeuroImage.
        Returns: init synaptic weigths, unweighted matrix (with 1s indicating edges and 0s otherwise)
        ...
        synaptic_weight_vector = struct_conn_vector * (
                1 + np.random.standard_normal((self.num_regions, self.num_regions)) * .001)

        # Adding synaptic mini-communities (within community 1)
        synaptic_weight_vector[0:numr_per_module // 2, numr_per_module // 2:numr_per_module] = synaptic_weight_vector[
                                                                                               0:numr_per_module // 2,
                                                                                               numr_per_module // 2:numr_per_module] * -0.2
        synaptic_weight_vector[numr_per_module // 2:numr_per_module, 0:numr_per_module // 2] = synaptic_weight_vector[
                                                                                               numr_per_module // 2:numr_per_module,
                                                                                               0:numr_per_module // 2] * -0.2
        synaptic_weight_vector[0:numr_per_module // 2, 0:numr_per_module // 2] = synaptic_weight_vector[
                                                                                 0:numr_per_module // 2,
                                                                                 0:numr_per_module // 2] * 1.2
        synaptic_weight_vector[numr_per_module // 2:numr_per_module,
        numr_per_module // 2:numr_per_module] = synaptic_weight_vector[numr_per_module // 2:numr_per_module,
                                                numr_per_module // 2:numr_per_module] * 1.2

        # MODIFICATION: 0 connectivity between structural community 1 and 3
        synaptic_weight_vector[0:numr_per_module, 2 * numr_per_module:3 * numr_per_module] = synaptic_weight_vector[
                                                                                             0:numr_per_module,
                                                                                             2 * numr_per_module:3 * numr_per_module] * 0
        synaptic_weight_vector[2 * numr_per_module:3 * numr_per_module, 0:numr_per_module] = synaptic_weight_vector[
